"""
Swiss Army knife that can perform all kind of operations on an allele frequency table:
...
Output: Various results can be derived, most central: a phylogenetic tree (using phylip)
...
Takes care of all kind of quirkyness:
    Population names limited in length, strange file format (fixed size delimited)
    Renaming populations back to readable names

can run my own implementation of Nei (1972) Distance
TODO: fix seqboot: needs to read some commands that make sure we use Gene frequencies
...
class AlleleFreqDict:
    """for a population, contains allele frequencies as a dict with the key as tuple like so:
                  ('A*74',): 0.011,
             ('A*80', '01'): 0.005,
             ('B*07', '02'): 0.063,
             ('B*08', '01'): 0.074,
...
class PopulationTree:
    def __init__(self, data="../Data/completeAFnet.csv", handSelection=False, precalc=False):
        ...
        if handSelection: ## expects a table with a column 'Selection' which has 'X' for populations to be used
    ...
    def inconsistentSubtypeAlleleFreqs(self):
        with open("../Data/alleleFreqDicts.pcl", 'rb') as f:
            afs = pickle.load(f)
        ## Creating allele frequency data structure, used to check if higher resolution subtypes are inconsistent with lower resolution
    ...
    def groupLoci(self, regex='([A-C])\d{1,3}'): ## About to be deprecated, see self.fix
        ## improved, was hackish which caused errors
        hlaGene = re.compile(regex) ## ideally be more precise and filter non-matches
    ...
    def runPhylipGenDist(self, basename, selectedLoci=['A','B','C'], selectedPops=True, skipTableCreate=False):
        os.chdir("../GenDist") ## TODO: put path into some global setting, create, if necessary

        if not skipTableCreate:
            # works if fix() is called, ie., col names are like A*24 etc
            selectedLociCols = [al for al in self.df.columns.values if al.split('*')[0] in selectedLoci]
            #sum([cols for locus, cols in self.groupings if locus in selectedLoci])
            if selectedPops:
                df = self.df[(self.df.Selection=='X') & (self.df.C > .85)][selectedLociCols] 
            else:
                df = self.df[selectedLociCols]
        ...
        os.system("rm outfile outtree")  # Friggin Phylip I/O leads to so much rubbish
